    ...
    <main>
      ...
      <section id="products" className="site-shell py-16 md:py-20 xl:py-24">
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {products.map((product) => (
            <motion.article key={product.title} className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm" whileHover={{ scale: 1.02 }}>
          ...
          <div className="grid gap-4 lg:grid-cols-8">
            {timeline.map((step, index) => (
              <div key={step} className="rounded-3xl border border-slate-200 bg-sand p-6 text-center">
                ...
                {index < timeline.length - 1 && <p className="mt-2 text-sm text-slate-500">&darr;</p>}
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="site-shell py-16 md:py-20 xl:py-24">
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          {stats.map((stat) => (
            <div key={stat.label} className="rounded-3xl bg-brand-green p-8 text-white shadow-lg">
